...
for p in $TCP_PORTS
do
    iptables -t nat -A PREROUTING -i $ni -p tcp --dport $p -j DNAT --to-destination 10.7.0.2:$p
    ip6tables -t nat -A PREROUTING -i $ni -p tcp --dport $p -j DNAT --to-destination fd07:dead:feed::2:$p  
done

iptables -t nat -A POSTROUTING -o $ni -j SNAT --to-source <1.2.3.4 - VPS Public IPv4>
ip6tables -t nat -A POSTROUTING -o $ni -j SNAT --to-source <2700::1234 - VPS Public IPv6>